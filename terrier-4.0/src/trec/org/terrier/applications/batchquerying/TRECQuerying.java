 ...
 * <h2>Configuring</h2>
 * <p>
 * In the following, we list the main ways for configuring TRECQuerying, before
 * exhaustively listing the properties that can affect TRECQuerying.
 *
 * <h3>Topics</h3>
 * Files containing topics (queries to be evaluated) should be set using the
 * <tt>trec.topics</tt> property. Multiple topic files can be used together by
 * separating their filenames using commas. By default TRECQuerying assumes TREC
 * tagged topic files, e.g.:
 * <pre>
 * &lt;top&gt;
 * &lt;num&gt; Number 1 &lt;/num&gt;
 ...
 * &lt;/top&gt;
 * </pre> If you have a topic files in a different format, you can used a
 * differed QuerySource by setting the property <tt>trec.topics.parser</tt>. For
 * instance
 * <tt>trec.topics.parser=SingleLineTRECQuery</tt> should be used for topics
 * where one line is one query. See
 * {@link org.terrier.applications.batchquerying.TRECQuery} and
 * {@link org.terrier.applications.batchquerying.SingleLineTRECQuery} for more
 * information.
 *
 * <h3>Models</h3>
 * By default, Terrier uses the {@link InL2} retrieval model for all runs. If
 * the <tt>trec.model</tt> property is specified, then all runs will be made
 * using that weighting model. You can change this by specifying another model
 * using the property <tt>trec.model</tt>. E.g., to use
 * {@link org.terrier.matching.models.PL2}, set <tt>trec.model=PL2</tt>.
 * Similarly, when query expansion is enabled, the default query expansion model
 * is {@link Bo1}, controlled by the property
 * <tt>trec.qe.model</tt>.
 *
 ...
 * <tt>trec.results</tt> property. If the property
 * <tt>trec.querycounter.type</tt>
 ...
 * not mult-process safe (eg one Terrier could delete it while another is
 * reading it).
 *
 *
 * <h2>Properties</h2>
 * <ul>
 * <li><tt>trec.topics.parser</tt> - the query parser that parses the topic
 * file(s). {@link TRECQuery} by default. Subclass the {@link TRECQuery} class
 * and alter this property if your topics come in a very different format to
 * those of TREC. </li>
 *
 * <li><tt>trec.topics</tt> - the name of the topic file. Multiple topics files
 * can be used, if separated by comma. </li>
 *
 * <li><tt>trec.model</tt> the name of the weighting model to be used during
 * retrieval. Default InL2 </li>
 * <li><tt>trec.qe.model</tt> the name of the query expansino model to be used
 * during query expansion. Default Bo1. </li>
 *
 * <li><tt>c</tt> - the term frequency normalisation parameter value. A value
 * specified at runtime as an API parameter (e.g. TrecTerrier -c) overrides this
 * property.
 *
 ...
 *
 * <li><tt>trec.manager</tt> the name of the Manager that is used for retrieval.
 ...
 *
 * <li><tt>trec.results.file</tt> the exact result filename to be output.
 * Defaults to an automatically generated filename - see
 * <tt>trec.querycounter.type</tt>.</li>
 *
 * <li><tt>trec.querycounter.type</tt> - how the number (RUNNO) at the end of a
 * run file should be generated. Defaults to sequential, in which case RUNNO is
 * a constantly increasing number. Otherwise it is a string including the time
 * and a randomly generated number.</li>
 *
 * <li><tt>trec.output.format.length</tt> - the very maximum number of results
 * ever output per-query into the results file . Default value 1000. 0 means no
 * limit.</li>
 *
 * <li><tt>trec.iteration</tt> - the contents of the Iteration column in the
 * trec_eval compatible results. Defaults to 0. </li>
 *
 * <li><tt>trec.querying.dump.settings</tt> - controls whether the settings used
 * to generate a results file should be dumped to a .settings file in
 * conjunction with the .res file. Defaults to true.
 *
 * <li><tt>trec.querying.outputformat</tt> - controls class to write the results
 * file. Defaults to {@link TRECDocnoOutputFormat}. Alternatives:
 * {@link TRECDocnoOutputFormat}, {@link TRECDocidOutputFormat}, {@link NullOutputFormat}</li>
 *
 * <li><tt>trec.querying.outputformat.docno.meta.key</tt> - for
 * {@link TRECDocnoOutputFormat}, defines the MetaIndex key to use as the docno.
 * Defaults to "docno".
 *
 * <li><tt>trec.querying.resultscache</tt> - controls cache to use for query
 * caching. Defaults to {@link NullQueryResultCache}</li>
 *
 * </ul>
 *
 * @author Gianni Amati, Vassilis Plachouras, Ben He, Craig Macdonald, Nut
 * Limsopatham
 */
public class TRECQuerying {
    ...
    /**
     * Added By Meltem 06032015 Values of features of terms (Mı, ChiSquare,
     * etc.)
     */
    protected Map featureValues_;

    /**
     * The number of results to output. Set by property
     * <tt>trec.output.format.length</tt>.
     */
    ...
    /**
     * Where results of the stream of queries are output to. Specified by
     * property <tt>trec.querying.outputformat</tt> - defaults to
     * TRECDocnoOutputFormat
     */
    protected OutputFormat printer;

    /**
     * results are obtained a query cache is one is enabled. Configured to a
     * class using property <tt>trec.querying.resultscache</tt>. Defaults to
     * NullQueryResultCache (no caching).
     */
    ...
    /**
     * TRECQuerying constructor initialises the inverted index, the lexicon and
     * the document index structures.
     */
    ...
    /**
     * TRECQuerying constructor initialises the inverted index, the lexicon and
     * the document index structures. Added By Meltem 30092014 QueryText &
     * QueryId'yi parametre olarak alır. Index fonksiyon çağırılmadan önce
     * yüklenmelidir.
     */
    public TRECQuerying(String queryText, String queryId, Index i) {
        ...
        this.printer = getOutputFormat();
        this.resultsCache = getResultsCache();
        this.queryexpansion = Boolean.parseBoolean(
            ApplicationSetup.getProperty("demir.qe", "false"));
    }

    public TRECQuerying(String queryText, String queryId, Index i, Map featureValues) {
    ...
    /**
     * Obtain the query cache. Loads the class specified by property
     * <tt>trec.querying.resultscache</tt>
     */
    ...
    protected void createManager() {
        try {
            ...
            /// Added/Modified By Meltem 06032016  
            if (managerName.equals("demir.terrier.querying.Manager")) {
                queryingManager = (Manager) (Class.forName(managerName)
                    .getConstructor(new Class[]{Index.class, Map.class})
                    .newInstance(new Object[]{index, featureValues_}));
            } else {
                queryingManager = (Manager) (Class.forName(managerName)
     ...
     *
     * @param predefinedName java.lang.String a non-standard prefix for the
     * result file.
     ...
     * @param query the query to process.
     * @param cParameter double the value of the parameter to use.
     * @param c_set A boolean variable indicating if cParameter has been
     * specified.
     */
     ...
     * <p>
     * <b>Queries</b><br />
     * Queries are parsed from file, specified by the <tt>trec.topics</tt>
     * property (comma delimited)
     *
     * @param c the value of c.
     * @param c_set specifies whether a value for c has been specified.
     * @return String the filename that the results have been written to Added
     * By Meltem Bu fonksiyon tek sorgu için result file oluşturmadan sorgu
     * sonucunu belirtilen formatta döndürür.
     */
    ...
    protected QuerySource getQueryParser() {
        ...
        try {
            ...
            if ((topicsFiles = ArrayUtils.parseCommaDelimitedString(ApplicationSetup.getProperty("trec.topics", ""))).length > 0) {
                ...
                logger.error("Error instantiating topic file.  Please set the topic file(s) using trec.topics property", new IllegalArgumentException());
            }
    ...
    /**
     * Get the query parser that is being used. This function does not search
     * for Topics file Only returns QuerySource
     *
     * @return The query parser that is being used. Added By Meltem
     */
    protected QuerySource getQueryParser(String sQueryText, String sQueryId) {
        QuerySource rtr = null;
        try {
            Class<? extends QuerySource> queryingClass = Class.forName(
                topicsParser.indexOf('.') > 0 ? topicsParser
            ...
            Object[] params = {sQueryText, sQueryId};
            rtr = queryingClass.getConstructor(types).newInstance(params);
